 ...
 *
 * Cache is bounded at CACHE_MAX entries with simple FIFO eviction.
...
export namespace TransformCache {
  ...
  // FIFO insertion-order map (Map preserves insertion order in JS).
  const _cache = new Map<string, unknown>()

  /**
   * Return the cached value for key if present, else call fn(), cache the
   * result, and return it.  FIFO eviction when size exceeds CACHE_MAX.
   */
  export function memo<T>(key: Key, fn: () => T): T {
    const k = keyOf(key)
    if (_cache.has(k)) return _cache.get(k) as T
