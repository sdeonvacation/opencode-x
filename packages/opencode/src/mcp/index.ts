...
export namespace MCP {
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      function monitor(s: State, name: string, client: MCPClient, mcp: Config.Mcp, maxRetries = 10) {
        let ctx: any
        try {
        ...
        client.onclose = () => {
          ...
          log.warn("transport closed", { server: name, retriesLeft: maxRetries })
                ...
                Effect.runPromise(
                  Effect.gen(function* () {
                    ...
                    if (!result.mcpClient) {
                      ...
                      // Retry with decremented budget
                      if (retries - 1 > 0) attempt(retries - 1)
                      return
                    }
                    s.status[name] = result.status
                    ...
                    // Reset retry budget on successful reconnect
                    monitor(s, name, result.mcpClient, mcp, maxRetries)
                  ...
                  log.warn("reconnect attempt failed", { server: name, error: String(err) })
                })
      ...
      function retryInitialConnection(s: State, name: string, mcp: Config.Mcp) {
        let ctx: any
        try {
          ctx = Instance.current
        } catch {}
        ...
        const attempt = (attemptNum: number) => {
          const delay = Math.min(2000 * Math.pow(2, attemptNum), 60000)
          if (attemptNum < 3) {
            log.info("scheduling initial connection retry", {
              server: name,
              attemptNum: attemptNum + 1,
              delayMs: delay,
            })
          } else {
            log.debug("scheduling initial connection retry", {
              server: name,
              attemptNum: attemptNum + 1,
              delayMs: delay,
            })
              ...
              Effect.runPromise(
                Effect.gen(function* () {
                  ...
                  if (!result.mcpClient) {
                    ...
                    const err = "error" in result.status ? result.status.error : ""
                    if (
                      err.includes("401") ||
                      ...
                      log.error("permanent auth error, stopping retry", { server: name, error: err })
                      return
                    }
                  ...
                  log.info("initial connection established", { server: name })
                  yield* bus.publish(ToolsChanged, { server: name }).pipe(Effect.ignore)
                }).pipe(Effect.ignore),
              ).catch((err) => {
                log.warn("initial connection attempt failed", { server: name, error: String(err) })
                attempt(attemptNum + 1)
              })
            if (ctx) Instance.restore(ctx, run)
            else run()
            ...
            ([key, mcp]) =>
              Effect.gen(function* () {
                ...
                if (result.mcpClient) {
                  ...
                  retryInitialConnection(s, key, mcp)
                }
              }),
