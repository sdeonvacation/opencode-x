...
function isTerminal(part: MessageV2.Part): boolean {
  // [fork-perf] hybrid-routing v4: compressed-output write marks `metadata.terminal === true`
  // to ensure DB persistence before next API turn dispatches
  if ((part as any).metadata?.terminal === true) return true
  if (part.type === "tool") {
