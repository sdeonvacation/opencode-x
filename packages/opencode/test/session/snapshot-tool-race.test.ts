  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir, llm }) {
        ...
        expect(diff.some((item) => item.file === "race-test.txt")).toBe(true)

        // Commit through the same session flow and ensure sidebar diff clears.
        yield* llm.toolMatch((hit) => JSON.stringify(hit.body).includes("commit the changes"), "bash", {
          command: 'git add race-test.txt && git commit --no-gpg-sign -m "commit race test"',
          description: "commit test file",
        })
        yield* llm.textMatch((hit) => JSON.stringify(hit.body).includes("commit the changes"), "done")

        yield* prompt.prompt({
          sessionID: session.id,
          agent: "build",
          noReply: true,
          parts: [{ type: "text", text: "commit the changes" }],
        })

        const committed = yield* prompt.loop({ sessionID: session.id })
        expect(committed.info.role).toBe("assistant")

        for (let i = 0; i < 50; i++) {
          diff = yield* Effect.promise(() => SessionSummary.diff({ sessionID: session.id }))
          if (!diff.some((item) => item.file === "race-test.txt")) break
          yield* Effect.sleep("100 millis")
        }
        expect(diff.some((item) => item.file === "race-test.txt")).toBe(false)
      }),
